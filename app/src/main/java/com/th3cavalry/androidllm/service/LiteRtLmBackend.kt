...
class LiteRtLmBackend(private val context: Context) : InferenceBackend {
    ...
    /**
     * Generates a streaming response for [prompt] on-device.
     * Emits tokens as they are generated by the LiteRT-LM engine.
     * Opens a fresh conversation per call (no state carried between iterations).
     */
    override fun generateStream(prompt: String): Flow<String> = flow {
        ...
        eng.createConversation().use { conversation ->
            conversation.sendMessageAsync(message).collect { chunk ->
                // Each chunk is a Message; extract and emit text content
                chunk.contents.filterIsInstance<Content.Text>()
