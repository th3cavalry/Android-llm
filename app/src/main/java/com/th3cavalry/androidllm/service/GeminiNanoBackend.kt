...
class GeminiNanoBackend(private val context: Context) : InferenceBackend {
     ...
     * For token-by-token streaming, use [generateStream] instead.
     */
    ...
    /**
     * Generates a streaming response using Gemini Nano.
     * Emits tokens as they are generated by the model.
     */
    override fun generateStream(prompt: String): Flow<String> = flow {
        val m = model ?: error("[Gemini Nano not initialized]")
        try {
            m.generateContentStream(prompt)
                .map { it.text ?: "" }
                .collect { token -> emit(token) }
        } catch (e: Exception) {
            emit("[Gemini Nano error: ${e.message}]")
        }
